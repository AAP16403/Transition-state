"""Diagnostic: What information does the model actually have access to?"""
import json
import numpy as np
...
for rxn_id, roles in sorted(reactions.items()):
    ...
    # Feature 1: Atom count
    feat = [n]
    
    # Feature 2: Atom composition (count of each type)
    atom_types = [a["atom"] for a in ts_e["atoms"]]
    for at in ["C", "H", "N", "O"]:
        feat.append(atom_types.count(at))
    
    # Feature 3: R-P difference statistics (how much the molecule changes)
    diff = c_R - c_P
    diff_norms = np.linalg.norm(diff, axis=1)
    feat.extend([diff_norms.mean(), diff_norms.std(), diff_norms.max(), diff_norms.min()])
    
    # Feature 4: Distance matrix statistics for R and P
    ...
    # Feature 5: Energies of R and P (if available)
    feat.append(r_e["energy"])
    feat.append(p_e["energy"])
    feat.append(abs(r_e["energy"] - p_e["energy"]) * 627.509)  # Reaction energy
...
# Test without energies (what the NN sees)
X_no_energy = X[:, :-3]
...
# Test with geometry + composition
X_geom_comp = X[:, :13]  
