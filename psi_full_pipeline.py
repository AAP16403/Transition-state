...
def ensure_sample_risk_features(sample, config, atom_types=None):
    ...
    if atom_types is None:
        ...
        raise KeyError(
            ...
            "so active_pair_mask cannot be rebuilt."
        )
    ...
    sample["active_pair_mask"] = torch.from_numpy(risk["active_pair_mask"])
...
def build_reaction_samples(config):
    ...
    try:
        ...
        raise ValueError(
            f"Failed to parse '{config['dataset_json']}'. The file is corrupted or incomplete. "
            f"Re-extract the dataset before training. Details: {e}"
        ) from None
